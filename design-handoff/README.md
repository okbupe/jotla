...
- **The adults around a child (the circle).** Onboarding grows an optional "The adults around them" section (their teacher, TA or club leader) and the child editor grows the same section: removable chips, case-insensitive dedupe, and a name still sitting in the box counts when Create or Done is pressed. The names live on the child object in this build's own localStorage shape (an `adults` array), so they ride the export and restore like everything else about the child. In child mode, the three adult-flavoured questions (classroom "Who was there?", playground "Which grown-ups were around?", and the extra-place "Who was there?") lead their chips with the child's own named adults, deduped against the generic words, so "who was with you" is one tap; the friends questions never carry them.
- **One information page.** The founder read the four info pages together and found them repeating the same promises, some verbatim. All informational content now lives on About Jotla (mission, live-now, dates trust, strong-record craft, the privacy promise said once, where the record lives, the three doors out, deleting, the coming board, Plus, feedback), and the What-Jotla-is-for / Privacy / Where-your-record-is-kept pages are deleted (Supersession Law); their old route names land on About so a saved navigation state never strands. The web-true claims stay exactly as this build's code has them: browser storage (no phone-backup claim), a live Restore from an export, photos and kept files travelling inside the export, videos never copied, files over 2 MB refused at pick time.
- **Settings decluttered.** The duplicated info rows, the planned-feature rows (Encrypted export, Lock the app: they explain themselves on About's coming board) and "Add another child" (the header avatar's profile sheet owns it) are gone. The backup card keeps the health line plus the real actions (Export, the live Restore); the profile card, Plus card, appearance controls, feedback card, privacy banner, tour row and About row (new sub: what it is, your privacy, where the record lives, what is coming) all stay.
- **"Your day" reads as working together.** On Plus the tile reads "Do it together with {name}" (the guided question cards are a two-of-you thing); Free keeps "Hand the phone to {name}". The tour's Today and child-mode slides are tier-neutral: "your child's own check-in", "Do it together, or hand the phone over."
- Suite grows Suite 12 (the circle walked end to end: onboarding, dedupe, pending-name-counts, editor round-trip, the child's cards leading with the named adults) plus the consolidation checks; 111/111 green.

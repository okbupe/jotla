...
function _mkRng(seed) { let s = seed >>> 0; return function () { s = (s * 1664525 + 1013904223) >>> 0; return s / 4294967296; }; }
...
const _HAND_TPL = [
  { behaviours: ['Refusing', 'Crying'], before: 'The bell went for the end of lunch.', during: 'Refused to line up and cried by the fence.', after: 'Came in holding the TA’s hand once the line had gone.', duration: '6 mins', helped: 'Waiting for the corridor to clear first.' },
  { behaviours: ['Running off', 'Withdrawing'], before: 'A supply teacher was covering the afternoon.', during: 'Ran to the cloakroom and sat under the pegs.', after: 'Returned with a quiet job to carry to the office.', duration: '12 mins', helped: 'Being given a job instead of an instruction.' },
  { behaviours: ['Crying', 'Stomping'], before: 'PE was moved to the hall without warning.', during: 'Stomped and cried at the changed plan.', after: 'Settled after sitting out the first five minutes.', duration: '10 mins', helped: 'A visual timetable check-in before the change.' },
  { behaviours: ['Hitting out', 'Crying'], before: 'A busy wet-play classroom at lunchtime.', during: 'Hit out when the game got too close and loud.', after: 'Calmed in the book corner with the lights low.', duration: '9 mins', helped: 'Space, low light and no questions until calm.' },
];
...
function _shiftStr(iso, days) { const p = iso.split('-').map(Number); const dt = new Date(p[0], p[1] - 1, p[2]); dt.setDate(dt.getDate() + days); return dt.getFullYear() + '-' + String(dt.getMonth() + 1).padStart(2, '0') + '-' + String(dt.getDate()).padStart(2, '0'); }
const _HOLIDAY_MONDAYS = ['2026-02-16', '2026-04-06', '2026-04-13']; // half term + Easter
function _inHoliday(iso) { return _HOLIDAY_MONDAYS.some(function (mon) { return iso >= mon && iso < _shiftStr(mon, 5); }); }
...
function _genHistory() {
  ...
  const clockIn = function (a, b) { const mins = Math.floor(a * 60 + rnd() * (b - a) * 60); return String(Math.floor(mins / 60)).padStart(2, '0') + ':' + String(mins % 60).padStart(2, '0'); };
  const moodPick = function (g, o) { const r = rnd(); return r < g ? 'good' : r < g + o ? 'ok' : 'hard'; };
  const kindPick = function () { return rnd() < 0.15 ? 'recalled' : 'contemporaneous'; };
  ...
  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    ...
    if (weekend || holiday) {
      if (rnd() < 0.28) {
        ...
        out.push({ id: 'e' + (n++), childId: 'sam', date: iso, time: rnd() < 0.5 ? 'Morning' : 'Afternoon', clock: clockIn(9, 18), setting: 'Home',
          category: pick(['Play', 'Eating', 'Other']), mood: mood, kind: kindPick(), type: 'quick', summary: pick(_HOME_TPL[mood]) });
      }
    } else {
      if (rnd() < 0.5) {
        const cat = pick(['Mornings', 'Mornings', 'Eating']);
        const mood = moodPick(0.55, 0.3);
        const e = { id: 'e' + (n++), childId: 'sam', date: iso, time: 'Morning', clock: clockIn(7.6, 9.2), setting: rnd() < 0.85 ? 'School' : 'Home',
          category: cat, mood: mood, kind: kindPick(), type: 'quick', summary: pick(_TPL[cat][mood]) };
      ...
      if (rnd() < 0.72) {
        ...
        const e = { id: 'e' + (n++), childId: 'sam', date: iso, time: 'Afternoon', clock: cat === 'Lunch hall' ? clockIn(12.9, 13.4) : clockIn(13, 16.5),
          setting: 'School', category: cat, mood: mood, kind: kindPick(), type: isHand ? 'handover' : 'quick', summary: pick(_TPL[cat][mood]) };
      ...
      if (dow === 5 && rnd() < 0.2) {
        ...
        out.push({ id: 'e' + (n++), childId: 'sam', date: iso, time: 'Afternoon', clock: clockIn(16, 17.5), setting: 'Club',
          category: 'Play', mood: mood, kind: kindPick(), type: 'quick',
    ...
    if (!weekend && !holiday && iso >= '2026-02-02' && rnd() < 0.38) {
      ...
      out.push({ id: 'm' + (n++), childId: 'maria', date: iso, time: rnd() < 0.4 ? 'Morning' : 'Afternoon', clock: clockIn(8.4, 15.7),
        setting: rnd() < 0.8 ? 'School' : 'Home', category: cat, mood: mood, kind: kindPick(), type: 'quick', summary: pick(_TPL[cat][mood]) });
    }
  }
  return out;
}
...
const SEED_DOCS = [
  { id: 'd05', childId: 'sam', title: 'Settling-in review meeting notes', type: 'Email', from: 'Oakfield Primary',
    received: '2026-01-09', about: 'Notes from the January settling-in review with the class teacher.', action: '', mood: 'good' },
  { id: 'd06', childId: 'sam', title: 'Request for an EHC needs assessment', type: 'Letter', from: 'Local Authority',
    received: '2026-01-14', about: 'Our request letter, sent recorded delivery. The 6-week clock starts here.', action: '', mood: 'good' },
  { id: 'd07', childId: 'sam', title: 'Acknowledgement of assessment request', type: 'Letter', from: 'Local Authority',
    received: '2026-01-21', about: 'The Local Authority confirming they received the request.', action: '', mood: 'good' },
  { id: 'd08', childId: 'sam', title: 'SEN support plan, spring term', type: 'Plan', from: 'Oakfield Primary',
    received: '2026-01-28', about: 'What the school says it will do this term and who does it.', action: 'Check it is happening half-termly', mood: 'ok' },
  { id: 'd09', childId: 'sam', title: 'GP referral to community paediatrics', type: 'Letter', from: 'GP',
    received: '2026-02-04', about: 'Referral following the January appointment.', action: '', mood: 'good' },
  { id: 'd10', childId: 'sam', title: 'Sensory checklist for home', type: 'Other', from: 'Occupational therapist',
    received: '2026-02-11', about: 'The checklist we filled in for the OT assessment.', action: '', mood: 'good' },
  { id: 'd11', childId: 'sam', title: 'Decision to assess', type: 'Letter', from: 'Local Authority',
    received: '2026-02-25', about: 'The Local Authority agreed to carry out the EHC needs assessment.', action: '', mood: 'good' },
  { id: 'd12', childId: 'sam', title: 'Parent advice form (our views)', type: 'Other', from: 'Local Authority',
    received: '2026-03-04', about: 'Our completed parental advice for the assessment, kept for reference.', action: '', mood: 'good' },
  { id: 'd13', childId: 'sam', title: 'Educational psychologist appointment', type: 'Letter', from: 'Local Authority',
    received: '2026-03-11', about: 'Date for the EP to observe Sam at school.', action: 'Confirm school knows', mood: 'ok' },
  { id: 'd14', childId: 'sam', title: 'Speech and language assessment report', type: 'Report', from: 'NHS Speech and Language',
    received: '2026-03-25', about: 'Assessment findings and a recommended programme.', action: '', mood: 'good' },
  { id: 'd15', childId: 'sam', title: 'Educational psychologist report', type: 'Report', from: 'Local Authority',
    received: '2026-04-22', about: 'The EP’s advice for the assessment, with recommendations.', action: '', mood: 'good' },
  { id: 'd16', childId: 'sam', title: 'Wet-play incident note', type: 'Email', from: 'Oakfield Primary',
    received: '2026-04-29', about: 'The school’s note about the lunchtime incident and what they changed.', action: '', mood: 'ok' },
  { id: 'd17', childId: 'sam', title: 'Occupational therapy report', type: 'Report', from: 'NHS Paediatric OT',
    received: '2026-05-06', about: 'Sensory profile and recommendations for the classroom.', action: '', mood: 'good' },
  { id: 'd01', childId: 'sam', title: 'EHC needs assessment decision', type: 'Letter', from: 'Local Authority',
    received: '2026-05-20', about: 'Agreement to issue an EHC plan; draft to follow.', action: 'Reply by 30 June', mood: 'good' },
  { id: 'd02', childId: 'sam', title: 'Draft EHC plan', type: 'Plan', from: 'Local Authority',
    received: '2026-06-02', about: 'The draft plan. 15 days to comment, so read Section F closely.', action: 'Comments due back', mood: 'ok' },
  ...
  { id: 'd18', childId: 'maria', title: 'Autumn review meeting notes', type: 'Email', from: 'Meadowbank Junior',
    received: '2026-01-16', about: 'Notes from the autumn term review, filed for reference.', action: '', mood: 'good' },
  { id: 'd19', childId: 'maria', title: 'Reading intervention plan', type: 'Plan', from: 'Meadowbank Junior',
    received: '2026-02-06', about: 'Three sessions a week with the reading tutor this term.', action: '', mood: 'good' },
  { id: 'd20', childId: 'maria', title: 'Hearing test results', type: 'Report', from: 'GP',
    received: '2026-03-06', about: 'Hearing within the normal range; query settled.', action: '', mood: 'good' },
  { id: 'd21', childId: 'maria', title: 'Maths anxiety support note', type: 'Email', from: 'Meadowbank Junior',
    received: '2026-04-24', about: 'What the class teacher is trying for the move into maths.', action: '', mood: 'ok' },
  { id: 'd22', childId: 'maria', title: 'School trip arrangements letter', type: 'Letter', from: 'Meadowbank Junior',
    received: '2026-05-13', about: 'Adjustments agreed for the science museum trip.', action: '', mood: 'good' },
...
const AVATAR_COLOURS = [
  ...
  { key: 'lavender', figure: '#9C8CEC' },
  { key: 'cocoa',    figure: '#A9744F' },
];

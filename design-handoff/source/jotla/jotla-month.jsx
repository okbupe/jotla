...
function PatternsLockedPreview({ onOpen }) {
  ...
  return (
    <button onClick={onOpen} className="j-press" data-patterns-locked aria-label="Month patterns, part of Jotla Plus"
      style={{ width: '100%', border: '1px solid color-mix(in srgb, var(--plus-ink) 30%, transparent)', borderRadius: 16,
        overflow: 'hidden', background: 'linear-gradient(var(--plus-tint), var(--plus-tint)) var(--card)',
        marginTop: 18, padding: 0, cursor: 'pointer', textAlign: 'left' }}>
      <div aria-hidden="true" style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 56, padding: '16px 16px 0' }}>
        {bars.map((h, i) => <span key={i} style={{ flex: 1, height: h, background: 'var(--plus-ink)', borderRadius: 4, opacity: i % 3 === 0 ? 0.5 : i % 3 === 1 ? 0.32 : 0.2 }} />)}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px 14px' }}>
        <span style={{ flex: 1, minWidth: 0 }}>
          <span style={{ display: 'block', fontFamily: "'Outfit', system-ui", fontWeight: 500, fontSize: 'calc(16px * var(--tscale, 1))', color: 'var(--ink)' }}>Month patterns</span>
          <span style={{ display: 'block', fontSize: 'calc(13px * var(--tscale, 1))', color: 'var(--muted)', marginTop: 2 }}>The mood graph, counts and hard-day patterns.</span>
        </span>
...
function MonthScreen({ nav, entries, view }) {
  ...
  const tween = (pairs, onDone) => {
    ...
    if (reduce) { pairs.forEach(([set, , to]) => set(to)); if (onDone) onDone(); return; }
    let t0 = null;
    const step = (ts) => {
      if (t0 === null) t0 = ts;
  ...
  // appears or goes.
  const fullHRef = React.useRef(0); fullHRef.current = fullH;
  const rowHRef = React.useRef(0); rowHRef.current = rowH;
  const graphHRef = React.useRef(0); graphHRef.current = graphH;
  React.useEffect(() => {
    ...
    const down = (ev) => {
      ...
      from = { y: ev.clientY, t0: tRef.current, span: Math.max(1, fullHRef.current - rowHRef.current) };
  ...
  window.__monthDebug = () => ({ t: tRef.current, g: gRef.current, dragging: draggingRef.current });
  ...
  React.useEffect(() => {
    ...
    const down = (ev) => {
      ...
      try { el.setPointerCapture(ev.pointerId); } catch (e) {}
      from = { y: ev.clientY, g0: gRef.current, t0: tRef.current,
  ...
  React.useEffect(() => {
    ...
    const trace = (m) => { if (window.__ladderTrace) window.__ladderTrace.push(m); };
    const down = (ev) => {
      if (el.scrollTop > 0) { trace('down-refused scrollTop=' + el.scrollTop); return; }
    ...
    const move = (ev) => {
      if (!from) { trace('move-no-from'); return; }
      const dy = ev.clientY - from.y;
      if (!from.engaged) {
        if (dy <= 0) { trace('move-disarm dy=' + dy.toFixed(1)); from = null; draggingRef.current = false; return; }   // ordinary scrolling
        if (dy <= 4) return;
        from.engaged = true;
        ...
        try { el.setPointerCapture(ev.pointerId); trace('capture-ok'); } catch (e) { trace('capture-fail ' + e.name); }
      ...
      setT(tNext);
      if (from.g0 < 1) { const gNext = Math.max(from.g0, Math.min(1, from.g0 + Math.max(0, d - usedT) / from.spanG)); trace('drive d=' + d.toFixed(1) + ' g=' + gNext.toFixed(3)); setG(gNext); }
    };
    const up = (ev) => {
      if (!from) return;
  ...
  const wxRef = React.useRef(0); wxRef.current = wx;
  ...
  const wkSettle = (fromX, toX, onDone) => {
    ...
    const reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (reduce) { setWx(toX); if (onDone) onDone(); return; }
    let t0 = null;
    const step = (ts) => {
      if (t0 === null) t0 = ts;
      const k = Math.min(1, (ts - t0) / 240);
      ...
      if (k < 1) wkTweenRef.current = requestAnimationFrame(step);
      else if (onDone) onDone();
    };
  ...
  React.useEffect(() => () => { if (wkTweenRef.current) cancelAnimationFrame(wkTweenRef.current); }, []);
  React.useEffect(() => {
    ...
    const down = (ev) => {
      ...
      from = { x: ev.clientX, y: ev.clientY, engaged: false, x0: committed ? 0 : wxRef.current,
        w: el.clientWidth || 1, pid: ev.pointerId };
    };
    const move = (ev) => {
      if (!from) return;
      ...
      if (!from.engaged) {
        ...
        if (Math.abs(dy) > 10 && Math.abs(dy) > Math.abs(dx)) { from = null; return; }
        if (Math.abs(dx) < 8 || Math.abs(dx) < Math.abs(dy) * 1.2) return;
        from.engaged = true;
        weekDraggedRef.current = true;
        try { el.setPointerCapture(from.pid); } catch (e) {}
      }
    ...
    const up = () => {
      ...
      if (!engaged) {
        ...
        if (wxRef.current !== 0) {
          ...
          setTimeout(() => { weekDraggedRef.current = false; }, 60);
        }
        return;
      }
      ...
      if (target && Math.abs(x) > w / 2) {
        ...
        wkPendingRef.current = () => { scrollToDate(target); setWx(0); };
  ...
  // week's landing day.
  const dayCellEl = (c, { key, anchor, interactive }) => {
  ...
  const weekCellsFor = (targetISO) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const iso = `${dt.getFullYear()}-${pad2(dt.getMonth() + 1)}-${pad2(dt.getDate())}`;
      const dayEntries = entries.filter(e => e.date === iso);
      cells.push({ d: dt.getDate(), iso, out: dt.getMonth() !== td.getMonth() || dt.getFullYear() !== td.getFullYear(),
        m: dt.getMonth(), y: dt.getFullYear(), mood: J.dayMood(dayEntries), count: dayEntries.length,
        future: iso > J.TODAY_ISO, isToday: iso === J.TODAY_ISO });
    }
    return cells;
  };
  ...
  const ghostWeekEl = (side) => {
    ...
    return (
      <div aria-hidden="true" data-week-ghost={side === 1 ? 'next' : 'prev'}
        style={{ position: 'absolute', top: weekRow * (rowH + GAP) * t, left: side === 1 ? '100%' : '-100%', width: '100%',
          display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: GAP, alignContent: 'start' }}>
        {weekCellsFor(target).map((c, i) => dayCellEl(c, { key: 'g' + side + '-' + i, anchor: target, interactive: false }))}
      </div>
    );
  };
  const pagerEl = (
            ...
            return (
              <div key={off} style={{ flex: '0 0 100%', width: '100%', scrollSnapAlign: 'start',
                ...
                {m.cells.map((c, idx) => {
                  ...
                  return dayCellEl(c, { key: c.iso, anchor: anchorISO, interactive: true });
                })}
              </div>
            );
          })}
          ...
          <div className="j-pad" style={{ paddingTop: 10 }}>
            ...
            <div className="j-card" data-cal-card style={{ padding: '14px 14px 0', overflow: 'hidden' }}>
              ...
              {/* the fold: the month, folded down to the anchor's week. The
                  ...
                  being read. Folded, the fold is also the week TRACK: the
                  whole thing translates with a horizontal finger, the ghost
                  weeks riding either side, and a drag must never also read
                  as a day tap. */}
              <div className="j-calfold" ref={foldRef} data-cal-fold style={{ height: foldH }}
                onClickCapture={e => { if (weekDraggedRef.current) { e.preventDefault(); e.stopPropagation(); } }}>
                <div data-week-track style={{ transform: `translateX(${wx}px)`, position: 'relative' }}>

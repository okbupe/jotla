...
- **The Colour and avatar sheet: 16 colours, 14 avatars, Done and Cancel (Bupe, 6 Aug).** Colour: the live app's 12 (`AVATAR_COLOURS`: amber, gold, orange, lime, green, teal, sky, indigo, slate, violet, plum, pink) plus four new hues the build must add: Mint `#3DBD8C`, Cyan `#1FA8D6`, Lavender `#9C8CEC`, Cocoa `#A9744F`. Still no red anywhere: red stays the alert colour. Sky is the default. Avatar: the live 10 glyphs (person, heart, star, leaf, sparkle, shield, bell, hand, house, note) plus four additions: the child's **initial** (the redesign default) and three new glyphs for the build, **moon, sun, music**. Both grids sit as two clean rows (30px swatches, 36px avatar discs). **This is the shell's third sheet type, the edit sheet**: taps preview live on the big avatar, **Done** commits and updates the row value, **Cancel** or the scrim reverts everything. (Pickers apply instantly; action sheets carry one button; edit sheets carry Done and Cancel.) **Avatars app-wide render as white initial-or-glyph on the chosen figure colour**, replacing the earlier accent-tint letter discs.

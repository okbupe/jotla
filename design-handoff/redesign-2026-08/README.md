...
- **The Colour and avatar sheet carries the live app's own palettes.** Colour: the real 12 swatches from `AVATAR_COLOURS` (amber, gold, orange, lime, green, teal, sky, indigo, slate, violet, plum, pink: no red, red is the alert colour), sky the default. Avatar: the live 10-glyph set (person, heart, star, leaf, sparkle, shield, bell, hand, house, note) plus the child's initial as a new eleventh option, the redesign's default. Selection ring in the text colour; a tap applies live (the board demos it on the big avatar and the row value). **Avatars app-wide now render as white initial-or-glyph on the chosen figure colour**, replacing the earlier accent-tint letter discs.

...
- **Menu**: the screen title is the child, avatar plus name, not the word "Menu"; tapping it switches or edits the child. **Jotla Plus is the first row and looks unique: a SOLID gold crown icon** (the only solid icon and the only gold in the app). Then Your record (**Backup and Restore**), App (**Theme**, showing just the current theme in grey, with three options: Light, Dark, System following the phone; **Text size**), About Jotla, then **Recycle Bin at the very last** (Bupe, 6 Aug: the live app's 15 Jul 30-day Bin feature, renamed Recycle Bin, sub "Kept for 30 days", bottom of the menu exactly as the old settings had it), and the "Jotla by SEN Help" endorsement as the footer.
